...
describe("generateFixedList", () => {
  it("parses a valid response into concepts with usage", async () => {
    const client = mockClient(
      JSON.stringify({
        concepts: [
          { name: "past simple", description: "Passado simples em inglês." },
          { name: "phrasal verb: called off", description: "Expressão que significa cancelar." },
        ],
      }),
    );

    const result = await generateFixedList([{ cardId: 1, front: "She called off the wedding.", back: "Ela cancelou o casamento." }], {
      client,
    });
  ...
  it("throws when the response is missing 'concepts'", async () => {
    ...
    await expect(generateFixedList([], { client })).rejects.toThrow(/missing 'concepts'/);
  });
  ...
  it("throws when the response text is empty", async () => {
    ...
    await expect(generateFixedList([], { client })).rejects.toThrow(/Empty response/);
  });
});
